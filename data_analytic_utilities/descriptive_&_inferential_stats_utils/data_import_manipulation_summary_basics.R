...
#Splitting a column of the dataframe. A single column can be split into two using separate function from tidyr package. Here for demonstration the 'Factor' word will be separated from the alphabets A-C into separate columns

data_analysis_split<-data_analysis%>%
    tidyr::separate(.,Factor,      # which column to split
                    c("Factor", "type"),  # the column names of the split columns
                    sep='_',    # the separater on which to split the columns
                    remove=TRUE) # remove the original colum

#View the result

head(data_analysis_split,5)

#Uniting two columns into a single column. Two different columns can be united into a single column using the unite function from tidyr package. Here the dataset generated by splitting the Factor column (above) will be used

data_analysis_unite<-data_analysis_split%>%
    tidyr::unite(Factor_type,  # the name of the new column
                 c("Factor", "type"),  # the names of the columns to be united
                 sep='_',    # separation between the united columns
                 remove=TRUE)  # remove the original columns

#View the result

head(data_analysis_unite)
...
#1. Inner Join (returns columns with matching values in the two datasets)

data_analysis_join<-dplyr::inner_join(data_analysis_9,
                                      data_analysis_11,
                                      by='Factor')

#View result

View(data_analysis_join)


#2. Left join (returns columns with all values of the left table and matching values from the right table)

data_analysis_lejoin<-dplyr::left_join(data_analysis_9,
                                       data_analysis_11,
                                       by='Factor')

#View result

View(data_analysis_lejoin)


#3. Full Join (returns columns with all values of from both the tables; NA's where there are no data is available)

data_analysis_fujoin<-dplyr::full_join(data_analysis_9,
                                       data_analysis_11,
                                       by='Factor')

#View result

View(data_analysis_fujoin)


#There are number of joins available through the dplyr package serving different purposes. More information on joins can be found on the following webpage: https://dplyr.tidyverse.org/reference/join.html
